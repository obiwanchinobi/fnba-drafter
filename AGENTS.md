...
- `Use built-it: <plan path>` selects the shared `built-it` workflow. Native invocation is `/built-it` in Grok/Claude and `$built-it` in Codex CLI/IDE. Read its `SKILL.md` before executing; it implements actioned rows from a spec-it plan via worktree-isolated subagents and serial cherry-pick. There is no `/ship-it` skill; the plan producer is `/spec-it`.
