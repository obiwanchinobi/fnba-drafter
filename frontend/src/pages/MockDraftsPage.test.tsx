...
test('Find winning weights posts the search, shows per-slot results and reloads collections', async () => {
  ...
  renderPage()
  fireEvent.click(
    await screen.findByRole('button', { name: 'Find winning weights' }),
  )
  ...
  expect(screen.getByRole('button', { name: /Run mock draft/ })).toBeDisabled()

  sets = result.slots.map((slot) => slot.weight_set)
  ...
  await waitFor(() => {
    expect(screen.getByRole('button', { name: 'Run mock draft' })).toBeEnabled()
  })

  const post = fetchMock.mock.calls.find(
    ([input, init]) =>
      String(input) === '/api/weight_searches' &&
      (init?.method ?? '').toUpperCase() === 'POST',
  )
  expect(post).toBeDefined()
  expect(
    fetchMock.mock.calls.filter(([input]) => String(input) === '/api/weight_sets'),
  ).toHaveLength(2)

  const combobox = screen.getByRole('combobox', { name: /weights/i })
