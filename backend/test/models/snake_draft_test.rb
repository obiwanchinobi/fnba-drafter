...
class SnakeDraftTest < ActiveSupport::TestCase
  ...
  test "reverses snake direction each round" do
    ...
    picks = SnakeDraft.new(order: order, board: board, rounds: 3).picks
  ...
  test "skips a top-ranked player when no open slot accepts that position" do
    ...
    picks = SnakeDraft.new(order: [ "Only" ], board: board, rounds: 13).picks
    ...
    assert_equal [ 13 ], board.map { |player| player[:player_id] }
  end
  ...
  test "assigns the earliest open slot: exact, then G or F/C, then UTIL, then BENCH" do
    ...
    pg_picks = SnakeDraft.new(order: [ "Only" ], board: pg_board, rounds: 7).picks
    ...
    pf_picks = SnakeDraft.new(order: [ "Only" ], board: pf_board, rounds: 3).picks
    ...
    c_picks = SnakeDraft.new(order: [ "Only" ], board: c_board, rounds: 2).picks
    assert_equal [ "C", "F/C" ], c_picks.map { |pick| pick[:roster_slot] }

    both = [ { player_id: 1, positions: [ "C", "PG" ], value: 10 } ]
    both_picks = SnakeDraft.new(order: [ "Only" ], board: both, rounds: 1).picks
  ...
  test "drafts eight teams across sixteen rounds and stops" do
    ...
    picks = SnakeDraft.new(order: order, board: board, rounds: League::ROUNDS).picks

    assert_equal 128, picks.size
    assert_equal (1..128).to_a, picks.map { |pick| pick[:overall_pick] }
    assert_equal [ 129, 130 ], board.map { |player| player[:player_id] }
